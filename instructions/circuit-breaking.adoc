...
[source,bash,subs="+macros,+attributes"]
----
$ oc exec -it -n istio-tutorial $(oc get pods -n istio-tutorial | grep recommendation-v2 | awk '{ print $1 }' | head -1) -c recommendation /bin/bash
----
...
[source,bash,subs="+macros,+attributes"]
----
$ oc patch deployment recommendation-v2 -p '{"spec":{"template":{"spec":{"containers":[{"name":"recommendation", "image":"quay.io/maistra_demos/recommendation:v2-timeout"}]}}}}' -n istio-tutorial
deployment.extensions/recommendation-v2 patched
----

Wait until the recommendation v2 pod has been patched and is ready:
[source,bash,subs="+macros,+attributes"]
----
$ oc -n istio-tutorial get pods
NAME                                 READY     STATUS    RESTARTS   AGE
customer-69b6b67bb7-sj25j            2/2       Running   0          5d9h
preference-v1-76684856fd-j8bdg       2/2       Running   0          5d9h
recommendation-v1-7f8755bb79-vjwq2   2/2       Running   0          5d9h
recommendation-v2-c6844bf-xkx5d      2/2       Running   0          19s
recommendation-v3-588747fd55-m8mj9   2/2       Running   0          5d9h
----
...
[source,bash,subs="+macros,+attributes"]
----
$ oc patch deployment recommendation-v2 -p '{"spec":{"template":{"spec":{"containers":[{"name":"recommendation", "image":"quay.io/maistra_demos/recommendation:v2"}]}}}}' -n istio-tutorial
deployment.extensions/recommendation-v2 patched
----
...
spec:
  ...
  - route:
    - destination:
        host: recommendation
    timeout: 1.000s
[root@localhost istio-lab-summit-2019]# cat src/istiofiles/virtual-service-recommendation-split.yml
apiVersion: networking.istio.io/v1alpha3
...
Note all recommendation in in the ms range
